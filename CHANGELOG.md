...
- Added [`adoption/`](./adoption/), a new companion-workstream directory that closes the six supporting adoption gaps named in the OECD assessment at the design-and-planning layer. It now includes a gap-closure plan, external-tools mapping annex, benchmark-harness design note, questionnaire-to-profile conversion design note, component-card format note, four published crosswalks, a crosswalk roadmap, and an OECD visibility plan.
- Added machine-readable component-card support under [`spec/`](./spec/): [`component-card.schema.json`](./spec/component-card.schema.json) defines the first JSON Schema for BRF component cards, and [`component-card-format.md`](./spec/component-card-format.md) explains how the cards connect to BR profiles and attestations.
- Published the first crosswalk deliverable, [`adoption/crosswalk-nist-ai-rmf.md`](./adoption/crosswalk-nist-ai-rmf.md), translating BRF into NIST AI RMF terms using official NIST AI RMF 1.0, AI RMF Core, Playbook, and Generative AI Profile sources. This is a translation-layer addition, not a framework-definition change.
- Added the first worked example component card, [`spec/examples/example-component-card-tool-legal-db.json`](./spec/examples/example-component-card-tool-legal-db.json), linked to the closed-world BR-2 example profile to make the new component-card schema concrete.
