...
This self-assessment is a **living document** tied to a framework version. When framework v0.6+ ships, or when any of Gap-1 through Gap-3 closes, this assessment is re-run. Self-assessment version: **v1.4** (2026-04-21; corrects A15 attestation — architectural demotion at decision path via tool-ontology binding + deployer policy library + schema-constrained enums + pack-bounded authority + deny-wins composition + provenance verification, documented in pact-public architecture docs. Residual surface narrower than v1.3 stated — only non-decision rendering/summary paths. Backlog item 10 reframed accordingly). Prior: v1.3 (PACT three-layer clarification), v1.2 (OBO A2A reference implementation citations), v1.1 (I5 correction), v1.0 (initial publication in framework v0.5.2).
